...
- <a id="doom-loop"></a>**Doom loop (boucle infernale)** — Quand l'IA corrige un bug, en crée un autre, re-corrige… sans fin. Le kit l'empêche avec la **Règle Preuve** et ses **3 tentatives** (3 échecs sur le même bug → on s'arrête, statut `BLOQUÉ` ; revenir au dernier point vert est une option que l'IA propose et que tu tranches). *Ex. : c'est la cause n°1 d'abandon — le kit te protège.*
