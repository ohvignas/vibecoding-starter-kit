...
- <a id="deux-apps"></a>**`site/` et `dashboard/` (stack vitrine)** — La vitrine est faite de **deux applications** dans le même dépôt : `site/` (Astro) publie les pages que Google et les IA lisent ; `dashboard/` (TanStack Start + Better Auth) est l'**admin privée** où tu écris le contenu. Les deux lisent la **même base Convex**, mais pas au même moment — voir [lecture au build](#build-time). *Ex. : tu écris un article dans l'admin, tu relances le build, il paraît.*
- <a id="build-time"></a>**Lecture au build (stack vitrine)** — Le contenu est lu **pendant la fabrication du site**, pas dans le navigateur du visiteur : le HTML part du serveur **déjà rempli**, donc Google et les IA le voient. La contrepartie : **publier ne suffit pas**, il faut **reconstruire le site** pour que la modification paraisse. *Ex. : `npm run build --workspace site`, puis tu redéploies.*
