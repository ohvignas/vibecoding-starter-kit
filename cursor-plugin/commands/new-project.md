...
1. Scaffold la stack choisie, **avec le preset shadcn** noté en Phase 5 :
   ...
   - **desktop** : `npx create-electron-app@latest <nom> --template=vite-typescript`. ⚠️ **Forge n'a pas de template React** (ses 5 templates : `base`, `vite`, `vite-typescript`, `webpack`, `webpack-typescript`) — le projet sort **sans React**. Ajoute-le avant shadcn, qui en dépend : `npm i react react-dom` + `npm i -D @vitejs/plugin-react @types/react @types/react-dom`, branche le plugin dans `vite.renderer.config.ts`, puis monte un `createRoot` dans le renderer. Ensuite seulement : `npx shadcn@latest init --base base --no-monorepo --yes`.
