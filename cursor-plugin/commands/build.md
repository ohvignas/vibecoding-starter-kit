...
7. **Coche** le jalon dans `docs/ROADMAP.md`, note tout piège dans `docs/memory/`, commit — puis pose un **point de restauration** : `git tag jalon-NN-<slug>` (NN = numéro du jalon) et **publie-le** : `git push --follow-tags`. Un tag resté en local ne sauve rien si la machine lâche. C'est le filet de `/sos` (retour à un état qui marche).
