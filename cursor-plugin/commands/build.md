...
7. **Coche** le jalon dans `docs/ROADMAP.md`, note tout piège dans `docs/memory/`, commit — puis pose un **point de restauration** : un tag **annoté**, `git tag -a jalon-NN-<slug> -m "jalon NN"` (NN = numéro du jalon), et **publie-le avec le commit** : `git push -u origin HEAD --follow-tags`. Le tag doit être **annoté** : `--follow-tags` ne pousse que ceux-là, un `git tag` nu resterait en local — et un tag resté en local ne sauve rien si la machine lâche. C'est le filet de `/sos` (retour à un état qui marche).
   - **`HEAD`, pas un nom de branche écrit d'avance** : il désigne la branche où tu es. Un projet neuf n'a que `main` ; un projet qui a plusieurs branches travaille sur celle que l'utilisateur a choisie, et le kit n'en décide pas à sa place.
   - **Synchronise AVANT de pousser** : `git fetch origin`, puis `git status`. Sans le `fetch`, git affiche `## main...origin/main` et tu **te crois à jour** alors qu'un autre chantier a poussé entre-temps ; après le `fetch`, il affiche `[behind 1]`. En retard → `git pull --rebase` (rejoue tes commits par-dessus, l'historique reste lisible), puis repousse.
   - **Si tu pousses quand même et que la course est perdue**, git répond `Updates were rejected because the tip of your current branch is behind`. Ce n'est pas une casse : `git pull --rebase` puis repousse. **Jamais `--force`** — tu écraserais le travail de l'autre chantier.
