...
4. ⚠️ **`PUBLIC_CONVEX_URL` et le `SITE_URL` public se passent AU BUILD de l'image du site — donc dans `build.args:`, jamais dans `environment:`.** Les pages publiques lisent Convex à ce moment-là ; `environment:` est l'endroit qui semble évident et il n'agit qu'au démarrage du conteneur, trop tard : l'image part **sans contenu**. Côté dashboard, `PUBLIC_CONVEX_URL` et `BETTER_AUTH_URL` vont bien dans `environment:` ; `BETTER_AUTH_SECRET` ne quitte jamais Convex.
