...
- **Après toute modif de `src/styles/*`** : **Accolades équilibrées** (autant de `{` que de `}`) ; le fichier servi (`curl` sur `app.css`) est du **vrai CSS**, pas une page d'erreur Vite/Tailwind ; puis le rendu (« Règle de vérification ») montre la couleur **primaire** de `docs/design.md`.
