...
## 6. Mise en ligne
> Prépare le déploiement Docker sur mon VPS : une image pour le build statique de `site/`, une pour le serveur de `dashboard/`, un reverse-proxy TLS devant. Liste les variables d'environnement à fournir. Et surtout : explique-moi **la boucle de publication** — ce qui se passe entre « je clique sur Publier » et « le visiteur voit la nouvelle page ». Dis-moi lequel des deux modèles tu proposes (reconstruire tout le site, ou purger la seule page publiée), et pourquoi — sans ça mon contenu changera dans Convex sans que personne ne le voie.
