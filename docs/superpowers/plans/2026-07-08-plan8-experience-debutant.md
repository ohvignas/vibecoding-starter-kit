...
> ⛔️ **PLAN EXÉCUTÉ ET PÉRIMÉ — NE PAS REJOUER.** Archive gardée pour la trace. Ses prescriptions ont été invalidées par le Lot D de `2026-07-27-remise-en-coherence.md` : la « **règle des 3 essais** » (`:414`, `:434`, `:448`-`:455`) n'existe pas sous ce nom — c'est la **Règle Preuve** et ses **3 tentatives** (`templates/agents/proof-rule.md`) ; le `git checkout <tag>` de `/sos` (`:436`) laisse une HEAD détachée et perd le travail — c'est désormais `git switch -c reprise-<tag> <tag>` ; la clé `STITCH_API_KEY` de `/doctor` (`:596`) n'a jamais existé (le MCP Stitch porte sa clé dans un en-tête). L'état courant fait foi : `templates/` et `docs/superpowers/plans/2026-07-27-remise-en-coherence.md`.
